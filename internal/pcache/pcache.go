...
func (pcache *PCache) PGetStruct(key string, res interface{}) error {
    data, err := pcache.PGet(key)
    if err != nil {
        return err
    }
...
func (pcache *PCache) PSetStruct(key string, val interface{}) error {
    jsonVal, err := json.Marshal(val)
    if err != nil {
        return err
    }
    pcache.PSet(key, jsonVal)
    return nil
}
...
func (pcache *PCache) isFileExist(key string) bool {
    path := pcache.filePath + "/" + pcache.md5(key) + ".cache"
